...
public class QuickFixes extends AbstractIntentionAction {
  ...
  private static final Map<Document, NavigableMap<Integer, IPopupChooserBuilder<QuickFix>>>
      lowerBound = new LinkedHashMap<>();
  private static final Map<Document, NavigableMap<Integer, IPopupChooserBuilder<QuickFix>>>
      upperBound = new LinkedHashMap<>();
  ...
  public void addDiagnostic(
      ...
      List<? extends Either<Command, CodeAction>> lenses) {
    ...
    IPopupChooserBuilder<QuickFix> popup =
        JBPopupFactory.getInstance().createPopupChooserBuilder(fixes);
  ...
  public void addFix(@NotNull Document doc, @NotNull Range range, QuickFix... newText) {
    ...
    Logger.getInstance(getClass())
        .info("fix from " + startOffset + " to " + endOffset + " of " + doc + ": " + newText);
    Logger.getInstance(getClass()).info("this: " + this);

    IPopupChooserBuilder<QuickFix> popup =
        JBPopupFactory.getInstance().createPopupChooserBuilder(Arrays.asList(newText));
  ...
  private Map.Entry<Integer, IPopupChooserBuilder<QuickFix>>[] getPopup(@NotNull Editor editor) {
    ...
    if (lowerBound.containsKey(doc) && lowerBound.get(doc).floorEntry(offset) != null) {
      if (upperBound.get(doc).ceilingEntry(offset) != null) {
        Map.Entry<Integer, IPopupChooserBuilder<QuickFix>> l =
            lowerBound.get(doc).floorEntry(offset);
        Map.Entry<Integer, IPopupChooserBuilder<QuickFix>> u =
            upperBound.get(doc).ceilingEntry(offset);
  ...
  public void invoke(@NotNull Project project, Editor editor, PsiFile psiFile)
    ...
    pp.setItemChosenCallback(
        s -> WriteCommandAction.runWriteCommandAction(project, () -> s.act(project)));

    pp.createPopup()
        .showInScreenCoordinates(editor.getComponent(), MouseInfo.getPointerInfo().getLocation());
  }
}
